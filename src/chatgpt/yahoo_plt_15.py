...
15. add These charts  candles then show the difference between the two averages at a glance.
"""
...
class StockData:
    ...
    def plot_data(self):
        try:
            ...
            # Create subplots
            fig = sp.make_subplots(rows=3, cols=1, shared_xaxes=True, row_heights=[0.5, 0.3, 0.2], vertical_spacing=0.02)
            ...
            fig.add_trace(go.Candlestick(x=self.data.index,
                                         ...
                                         name='Price'),
                          row=1, col=1)
            fig.add_trace(go.Scatter(x=self.data.index, y=self.data['12_EMA'], mode='lines', name='12 EMA', line=dict(color='blue')), row=1, col=1)
            fig.add_trace(go.Scatter(x=self.data.index, y=self.data['26_EMA'], mode='lines', name='26 EMA', line=dict(color='red')), row=1, col=1)
            
            # MACD chart
            fig.add_trace(go.Scatter(x=self.data.index, y=self.data['MACD'], mode='lines', name='MACD', line=dict(color='purple')), row=2, col=1)
            fig.add_trace(go.Scatter(x=self.data.index, y=self.data['Signal'], mode='lines', name='Signal', line=dict(color='orange')), row=2, col=1)
            fig.add_trace(go.Bar(x=self.data.index, y=self.data['MACD_Histogram'], name='MACD Histogram', marker_color='gray'), row=2, col=1)
            
            # Buy/Sell volume
            fig.add_trace(go.Bar(x=self.data.index, y=self.data['Buy Volume'], name='Buy Volume', marker_color='green'), row=3, col=1)
            fig.add_trace(go.Bar(x=self.data.index, y=self.data['Sell Volume'], name='Sell Volume', marker_color='red'), row=3, col=1)
            
            # Set layout for interactive cursor alignment
            fig.update_layout(title=f"Stock Price and MACD for {self.ticker}", hovermode='x unified', showlegend=True)
