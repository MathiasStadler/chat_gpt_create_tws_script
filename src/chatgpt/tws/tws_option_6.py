...
class OptionChainFetcher:
    ...
    def print_option_chain(self, option_chain):
        try:
            print(f"{'Strike Price':^20} | {'Call Left':^20} | {'Put Right':^20}")
            print('-' * 60)
            for option in option_chain:
                strike_price = option['Strike']
                call_left = f"Ask: {option['Ask Price']} | Mid: {option['Mid Price']} | Bid: {option['Bid Price']}" if option['Right'] == 'C' else ''
                put_right = f"Ask: {option['Ask Price']} | Mid: {option['Mid Price']} | Bid: {option['Bid Price']}" if option['Right'] == 'P' else ''
                print(f"{strike_price:^20} | {call_left:^20} | {put_right:^20}")
        except Exception as e:
...
def main():
    ...
    if option_chain:
        fetcher.print_option_chain(option_chain)  # Print option data in formatted output
