...
log_filename = f"{log_directory}/tws_positions_{current_time}.log"
...
class IBPositions:
    ...
    async def disconnect(self):
        try:
            if self._connected and self.ib.isConnected():
                ...
                await asyncio.sleep(1)  # Give time for pending operations
                self.ib.disconnect()
                self._connected = False
        except Exception as e:
    ...
    def _create_position_table(self):
        ...
        # Save table to file
        table_filename = f"{log_directory}/positions_table_{current_time}.txt"
