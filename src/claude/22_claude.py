    ...
    async def connect(self):
        try:
            if not self._connected:
                ...
                # Set market data type globally at start
                self.ib.reqMarketDataType(MarketDataTypeEnum.DELAYED)
                logger.info("Set market data type to DELAYED")
                await asyncio.sleep(1)
        ...
        for item in portfolio:
            try:
                ...
                if contract_details:
                    ...
                    logger.info(f"Requesting delayed market data for {contract.symbol}")
                    
                    # Request delayed market data with more specific parameters
                    ticker = self.ib.reqMktData(contract, 
                        genericTickList='100,101,104,106,165,221,232,236',
                        snapshot=False,
                    ...
                    for _ in range(5):  # Try up to 5 times
                        ...
                        # Check delayed data fields specifically
                        market_price = (ticker.delayed_last or 
                                      ticker.delayed_close or 
                                      ticker.delayed_bid or 
                                      ticker.delayed_ask or 
                                      0.0)
                        
                        if market_price > 0:
                            logger.info(f"Received delayed price for {contract.symbol}: {market_price}")
                            break
                    
                    market_value = market_price * item.position
                    ...
                    print(f"Market Price (Delayed): ${market_price:.2f}")
...
if __name__ == "__main__":
    ...
    # Warning 10167, reqId 10: Requested market data is not subscribed. Displaying delayed market data