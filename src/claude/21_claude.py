        ...
        for item in portfolio:
            try:
                ...
                if contract_details:
                    ...
                    await asyncio.sleep(3)  # Wait for delayed data
                    
                    market_price = ticker.last if hasattr(ticker, 'last') and ticker.last else \
                                 ticker.close if hasattr(ticker, 'close') and ticker.close else \
                                 ticker.delayed_last if hasattr(ticker, 'delayed_last') and ticker.delayed_last else 0.01
