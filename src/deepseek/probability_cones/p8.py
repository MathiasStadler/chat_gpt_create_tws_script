...
class StockAnalyzer:
    ...
    def calculate_greeks(self, row, underlying_price, risk_free_rate=0.01):
        try:
            # Parse expiration date from contractSymbol (e.g., AAPL220318C00150000)
            expiration_str = row['contractSymbol'][4:10]  # Extract YYMMDD
            ...
            theta = (-(S * norm.pdf(d1) * sigma) / (2 * np.sqrt(T)) - r * K * np.exp(-r * T) * norm.cdf(d2)) / 365 if row['Option Type'] == 'Call' else (-(S * norm.pdf(d1) * sigma) / (2 * np.sqrt(T)) + r * K * np.exp(-r * T) * norm.cdf(-d2)) / 365
            vega = S * norm.pdf(d1) * np.sqrt(T) / 100
            rho = K * T * np.exp(-r * T) * norm.cdf(d2) / 100 if row['Option Type'] == 'Call' else -K * T * np.exp(-r * T) * norm.cdf(-d2) / 100
