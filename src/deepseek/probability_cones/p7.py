...
class StockAnalyzer:
    ...
    def calculate_greeks(self, row, underlying_price, risk_free_rate=0.01):
        try:
            ...
            expiration_date = datetime.fromtimestamp(row['expirationDate'])  # Convert timestamp to datetime
            current_date = datetime.now()
            days_to_expiration = (expiration_date - current_date).days

            # Calculate Greeks using the Black-Scholes model (via mibian)
            option = mibian.BS(
                [underlying_price, row['strike'], risk_free_rate, days_to_expiration],
                callPrice=row['lastPrice'] if row['Option Type'] == 'Call' else None,
                putPrice=row['lastPrice'] if row['Option Type'] == 'Put' else None,
                volatility=row['impliedVolatility'] * 100  # Convert to percentage
            )
            return pd.Series({
                'Delta': option.callDelta if row['Option Type'] == 'Call' else option.putDelta,
                'Gamma': option.gamma,
                'Theta': option.callTheta if row['Option Type'] == 'Call' else option.putTheta,
                'Vega': option.vega,
                'Rho': option.callRho if row['Option Type'] == 'Call' else option.putRho
            })
        except Exception as e:
