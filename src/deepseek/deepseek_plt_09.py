    ...
    def plot_data(self):
        try:
            ...
            fig = make_subplots(
                ...
                subplot_titles=("Stock Prices", "Trading Volume"), row_heights=[0.7, 0.3]
            )
            
            # Add Close Price Line Chart
            fig.add_trace(go.Scatter(x=self.data.index, y=self.data['Close'], mode='lines', name='Close Price', hoverinfo='x+y'), row=1, col=1)
            
            # Add Volume as Bar Chart
            fig.add_trace(go.Bar(x=self.data.index, y=self.data['Volume'], name='Volume', hoverinfo='x+y'), row=2, col=1)
            
            # Update layout to remove space and add interactive cursor
            fig.update_layout(
                title_text=f"Stock Prices and Volume for {self.ticker}",
                xaxis_title="Date",
