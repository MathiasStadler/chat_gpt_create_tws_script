...
13. get all next upcoming earnings date with quote
14. please use the paper account on localhost 127.0.0.1 with port 7496 and session id 33)
"""
...
class MarketDataFetcher:
    ...
    def get_earnings_dates(self, symbol):
        ...
        try:
            contract = Stock(symbol, 'SMART', 'USD')
            self.ib.qualifyContracts(contract)
            # Use a valid reportType for fundamental data
            report_type = 'ReportsFinSummary'  # Financial summary report
            earnings = self.ib.reqFundamentalData(contract, report_type)
            return earnings
        except Exception as e:
...
def main():
    try:
        ...
        # Fetch upcoming earnings dates (skip if not available)
        earnings = None
        try:
            ...
            print(f"Skipping earnings data due to error: {e}")
